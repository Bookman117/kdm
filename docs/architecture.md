...
Phase 1 只建立最上層 CLI 與 `lib/` 安全基線，不呼叫節點或 Kubernetes API。
...
- [ ] 第一個正式支援 OS：Ubuntu 22.04 或 24.04？
- [ ] 第一個 integration lab：Multipass、Vagrant/libvirt、Proxmox 或既有 VM？
- [ ] inventory 使用 YAML 時採 `yq`、Ruby/Python parser，或限制為 shell-readable config？
